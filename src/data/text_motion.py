...
def read_split(path, split, splits_dir=None):
    """
    Read split file from either custom splits directory or default location.
    
    Args:
        path: Base dataset path
        split: Split name (train/val/test)
        splits_dir: Custom splits directory path (optional)
    """
    if splits_dir is not None:
        split_file = os.path.join(splits_dir, split + ".txt")
    else:
...
class TextMotionDataset(Dataset):
    def __init__(
        ...
        self.split = split
        self.keyids = read_split(path, split, splits_dir)
